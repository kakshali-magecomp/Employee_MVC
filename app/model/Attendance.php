...
class Attendance
{
    ...
    public function getTodayAttendance($employeeId)
    {
        ...
        $sql = "SELECT *FROM attendance WHERE employee_id = ? AND attendance_date = ? LIMIT 1";
    ...
    public function punchIn($employeeId)
    {
        ...
        $status = "Present";
        $sql = "INSERT INTO attendance (employee_id, attendance_date, punch_in, status) VALUES (?, ?, ?, ?)";
        $stmt = $this->conn->prepare($sql);
    ...
    public function punchOut($employeeId)
    {
        ...
        $sql = "UPDATE attendance SET punch_out = ?, working_hours = ? WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
        if (!$stmt)
        {
            die("Prepare Failed : " . $this->conn->error);
        ...
        $stmt->bind_param("ssi", $punchOutTime, $totalWorkingHours, $attendance['id']);
    ...
    {
        $sql = "SELECT *FROM attendance WHERE employee_id = ? ORDER BY attendance_date DESC";
        $stmt = $this->conn->prepare($sql);
    ...
    public function getMonthlySummary($employeeId)
    {
        $month = date('m');
        $year = date('Y');
        $sql = "SELECT COUNT(*) as total_present FROM attendance WHERE employee_id = ? AND MONTH(attendance_date) = ? AND YEAR(attendance_date) = ?";
        $stmt = $this->conn->prepare($sql);
        if (!$stmt)
        {
            die("Prepare Failed : " . $this->conn->error);
    ...
    {
        $sql = "SELECT attendance.*, Employee.full_name, Employee.email FROM attendance JOIN Employee
                ON attendance.employee_id = Employee.id ORDER BY attendance.attendance_date DESC";
    ...
    {
        $sql = "SELECT attendance.*, Employee.full_name, Employee.email FROM attendance
                JOIN Employee ON attendance.employee_id = Employee.id WHERE 1=1";
    ...
    {
        $sql = "SELECT attendance.*, Employee.full_name, Employee.email FROM attendance JOIN Employee ON attendance.employee_id = Employee.id
