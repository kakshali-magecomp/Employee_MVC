...
<div class="max-w-7xl mx-auto p-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div>
                <?php if(!empty($employee['profile_image'])): ?>
                    <img src="/public/upload/<?= $employee['profile_image'] ?>" class="w-32 h-32 rounded-full object-cover border-4 border-indigo-500 shadow-lg">
                <?php else: ?>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white   p-6">
            ...
            <p class="text-3xl font-bold text-green-500 mt-3">
                <?= $punchIn ? date('h:i:s A', strtotime($punchIn)) : 'Not Punched In'?>
            </p>
        </div>
        <div class="bg-white  p-6">
            <h3 class="text-gray-500 text-lg">
            ...
            <p class="text-3xl font-bold text-red-500 mt-3">
                ...
                <?= $punchOut ? date('h:i:s A', strtotime($punchOut)) : 'Not Punched Out'?>
            </p>
    ...
    <div class="bg-white rounded-3xl  p-8 mb-8">
        ...
        <div class="flex flex-wrap gap-5">
            <a href="index.php?page=punchin" class="bg-green-500 hover:bg-green-600 text-white px-8 py-4 rounded-2xl text-lg font-semibold transition shadow-lg">
                Punch In
            </a>
            <a href="index.php?page=punchout" class="bg-red-500 hover:bg-red-600 text-white px-8 py-4 rounded-2xl text-lg font-semibold transition shadow-lg">
                Punch Out
            </a>
        </div>
    </div>
                    ...
                    <?php if(!empty($attendanceHistory)): ?>
                        <?php foreach($attendanceHistory as $row): ?>
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="p-4">
                                    <?= $row['punch_in'] ? date('h:i:s A', strtotime($row['punch_in'])) : '-'?>
                                </td>
                                <td class="p-4">
                                    <?= $row['punch_out'] ? date('h:i:s A', strtotime($row['punch_out'])) : '-' ?>
                                </td>
